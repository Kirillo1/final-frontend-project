...
const defaultIconVariants = {
    info: <InfoOutlinedIcon className="w-6 h-6 text-blue-600" />,
    warning: <WarningAmberRoundedIcon className="w-6 h-6 text-amber-600" />,
    success: <CheckRoundedIcon className="w-6 h-6 text-emerald-600" />,
    error: <ErrorOutlineRoundedIcon className="w-6 h-6 text-rose-600" />,
};
...
const Alert = ({
    ...
    return (
        <div
            id="Alert"
            className={`inline-flex transform-gpu transition-transform duration-500 ease-in-out items-center ${variantClasses[variant]
                } ${alignClasses[align]} ${isOpen ? "translate-y-0" : "translate-y-96"
        ...
        >
            ...
            <button className="absolute right-2 top-2" onClick={onClose}>
                <CloseRoundedIcon className="w-6 h-6 fill-zinc-800" />
            </button>
        </div>
    );
};
