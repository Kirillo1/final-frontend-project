...
const Header = () => {
  ...
  const favoritesCount =
    (favorites?.smartphones?.length || 0) +
    (favorites?.accessories?.length || 0);

  const { cartProducts } = useCartProducts();
  const cartProductsCount =
    cartProducts.smartphones.length + cartProducts.accessories.length;
  ...
  const allErrorsAreNull = Object.values(registrationFormErrors).every(
    (value) => value === null
  );
  ...
  return (
    <header className="bg-black shadow-lg shadow-violet-700">
      ...
      >
        <NavLink
          to="/"
          className="text-white text-xl flex-shrink-0 flex items-center"
        >
          MobileGuru
        </NavLink>
        ...
        {showLoginModal && (
          ...
          >
            <form onSubmit={handleLoginForm} className="text-center">
              <Input
                label="Ваша почта"
                name="login"
                type="email"
                    ...
                    {products.map((item) => (
                      ...
                      >
                        <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg">
                          <item.icon
                            aria-hidden="true"
                            className="h-6 w-6 group-hover:text-violet-500"
                          />
                        </div>
                        <div className="flex-auto">
                          <a
                            href={item.href}
                            className="block font-semibold text-white hover:text-violet-500"
                          >
                            {item.name}
